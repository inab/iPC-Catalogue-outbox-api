...
class TokenVerify extends Middleware
{
	...
	public function __invoke($request, $response, $next)
	{
		...
		$userLogin = $this->oauth2->getAttrFromTokenResponse("sub", $r);
		if (empty($userLogin)) {
			$code = 406;
			$errMsg = "Token successfully authorized, but resource owner retrieval failed. No 'sub (ID)'";
			throw new \Exception($errMsg, $code);
		}

		$request = $request->withAttribute('userLogin', $userLogin);

		$userEmail = $this->oauth2->getAttrFromTokenResponse("email", $r);
		if (empty($userEmail)) {
			$code = 406;
			$errMsg = "Token successfully authorized, but resource owner retrieval failed. No 'email'";
			throw new \Exception($errMsg, $code);
		}
